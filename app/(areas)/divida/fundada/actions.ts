 ...
 */
export async function acaoDeDividaFundadaAction(
  _prev: EstadoDoMolde,
  formData: FormData
  ...
  try {
    if (acao === "criar") {
      await criarDividaFundada(campos);
    } else {
  ...
  if (id !== "") revalidatePath(`${DIVIDA_FUNDADA.rota}/${id}`);
  return { sucesso: acao === "criar" ? "Dívida cadastrada." : "Registro gravado." };
}
