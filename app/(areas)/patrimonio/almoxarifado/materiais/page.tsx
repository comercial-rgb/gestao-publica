...
import { listarMateriais, opcoesDoAlmoxarifado } from "../../../../../lib/portas/recursos/almoxarifado-dados";
import { PortaSemBancoError } from "../../../../../lib/portas/recursos/dados";
import { acaoDeMateriaisAction } from "./actions";
...
export default async function Pagina({
  ...
  await exigirLeitura();
  const consulta = lerConsulta(MATERIAIS, await searchParams);

  try {
    const [pagina, opcoes, permitidas] = await Promise.all([
      listarMateriais(consulta),
      opcoesDoAlmoxarifado(),
      acoesPermitidas([
        ...Object.values(MATERIAIS.permissoes).filter((p): p is string => p !== undefined),
        ...MATERIAIS.acoes.map((a) => a.acaoDoCenso),
      ]),
    ]);

    const somaveis = MATERIAIS.colunas.filter((c) => c.somavel === true).map((c) => c.nome);
    ...
    if (e instanceof PortaSemBancoError) {
      return (
        <div className="space-y-6">
          <PageHeader titulo={MATERIAIS.rotulo} subtitulo={MATERIAIS.descricao} />
          <EstadoVazio
