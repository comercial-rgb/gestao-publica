...
import { criarClasseDeMaterial, semAcaoDeDetalhe } from "../../../../../lib/portas/recursos/almoxarifado-dados";
 ...
 */
export async function acaoDeClassesAction(
  _prev: EstadoDoMolde,
  formData: FormData
  ...
  try {
    if (acao === "criar") {
      await criarClasseDeMaterial(campos);
    } else {
  ...
  revalidatePath(CLASSES_DE_MATERIAL.rota);
  if (id !== "") revalidatePath(`${CLASSES_DE_MATERIAL.rota}/${id}`);
  return { sucesso: acao === "criar" ? "Classe cadastrada." : "Registrado." };
}
