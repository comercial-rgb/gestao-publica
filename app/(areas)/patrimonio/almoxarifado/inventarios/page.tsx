...
import { listarInventariosDeEstoque, opcoesDoAlmoxarifado } from "../../../../../lib/portas/recursos/almoxarifado-dados";
import { PortaSemBancoError } from "../../../../../lib/portas/recursos/dados";
import { acaoDeInventariosAction } from "./actions";

/**
 * Inventarios De Estoque — tela do MOLDE (ENT06). O ENT05 deu a esta seção modelo, caso de uso e
...
export default async function Pagina({
  ...
  try {
    const [pagina, opcoes, permitidas] = await Promise.all([
      listarInventariosDeEstoque(consulta),
      opcoesDoAlmoxarifado(),
      acoesPermitidas([
        ...Object.values(INVENTARIOS_DE_ESTOQUE.permissoes).filter((p): p is string => p !== undefined),
        ...INVENTARIOS_DE_ESTOQUE.acoes.map((a) => a.acaoDoCenso),
      ]),
    ]);

    const somaveis = INVENTARIOS_DE_ESTOQUE.colunas.filter((c) => c.somavel === true).map((c) => c.nome);
    ...
    if (e instanceof PortaSemBancoError) {
      return (
        <div className="space-y-6">
          <PageHeader titulo={INVENTARIOS_DE_ESTOQUE.rotulo} subtitulo={INVENTARIOS_DE_ESTOQUE.descricao} />
          <EstadoVazio
